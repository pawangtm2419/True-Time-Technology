...
if( isset( $_POST["delete"] ) ) {
   ...
   {
   $deletequery = mysql_query("delete from customerorder where id = '$idd'");
   header("Location:order-mgmt.php");
   }
}
$id = $_REQUEST['id'];
if($id)
{
$deletequery = mysql_query("delete from customerorder where id = '$id'");
header("Location:order-mgmt.php");
}
$act = $_REQUEST['act'];
if($act)
{
$updatestatus = mysql_query("update customerorder set status = '0' where id = '$act'");
}
$dact = $_REQUEST['dact'];
if($dact)
{
$updatestatus = mysql_query("update customerorder set status = '1' where id = '$dact'");
}
?>
 ...
 <div class="menu_box" >
  <ul>
      ...
      <li><a href="welcome.php">Home</a></li>
       <li><a href="order-mgmt.php">Manage Order</a></li>
  </ul>
 </div> 
           <div style="width:900px; background:#0099FF; margin:0 auto; line-height:40px; margin-left:50px;">
            ...
            <form name="frm1" method="post">
                ...
                <span class="block" style="width:200px; ;">Order Id</span>
                <span class="block" style="width:200px; ;">First Name</span>
                <span class="block" style="width:150px; ">Mobile</span>
                <span class="block" style="width:50px; ">Payment</span>
			...
			if($searchproduct)
			{
			$query = "select * from `customerorder` where fname LIKE '%$searchproduct%' order by id desc";
			}
			else
			{
            $query = "select * from `customerorder` order by id desc";
			}
	    ...
	    ?>
            <div class="grid-box-data" style="width:898px;float:right;">
                ...
                <span class="block" style="width:200px; ;">
                  <?php echo $select[$i]["orderid"];?></span>
              <span class="block" style="width:200px;" ><?php echo $select[$i]["fname"];?></span>
              <span class="block" style="width:150px;">
                 <?php echo $select[$i]["mobile"];?></span>
                 <span class="block" style="width:50px;">
                 <?php echo $select[$i]["paymentstatus"];?></span>
                 
             
              <div class="apply-icons-1" style="width:12%;">
                 <ul>
                        <li><a href="customerorder-mgmt.php?id=<?php echo $select[$i]["id"];?>" style="background-image: none;float:left;"><img src="images/delate-icon.png" alt="Delete" title="Delete" /></a></li>
        <li><a href="order-detail.php?id=<?php echo $select[$i]["id"];?>" rel="facebox" style="background-image: none;float: left;">View</a></li>
                         </ul>
    </div></div>
